...
- **CSV upload and validation**  
  The `core` app exposes an endpoint `/api/upload-equipment/` that accepts a CSV file
...
- **Analytics with Pandas**  
  After reading the CSV, the backend:
...
- **PDF report generation and storage**  
  The summary statistics are also written into a simple PDF report using **ReportLab**.
...
- **History management (keep last 5 uploads)**  
  Each successful upload creates a `History` entry containing:
...
I tried to keep the code as close as possible to a normal student project:
short modules, straightforward naming and a few TODOs in places where I
would probably extend the project if given more time.
